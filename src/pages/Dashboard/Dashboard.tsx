...
import { bcsStats, useActivePetBcs } from '@domains/bcs'
import { compareAgainstRecommended, monthBreakdown, useActivePetBudget } from '@domains/budget'
import { useCareGuide } from '@domains/care-guides'
import { useCommunitiesList } from '@domains/communities'
import { diaryStats, useActivePetDiary, type DiaryCategory } from '@domains/diary'
import {
  useExternalLinks,
  type SpeciesCategory as ExtSpeciesCategory,
} from '@domains/external-links'
import { useFuneralList } from '@domains/funeral'
import { useActivePetPhotos } from '@domains/gallery'
import { detectBreaches, recommendationFor, useActivePetHabitat } from '@domains/habitat'
import { upcomingDues, useActivePetHealth, weightTrend } from '@domains/health'
...
function Dashboard() {
  ...
  return (
    <section className={styles.dashboard}>
      <header className={styles.header}>
        ...
        {species ? (
          ...
          <EmptyState
            variant="gated"
            icon="🐾"
            className={styles.firstRunPrimer}
            title={t('dashboard.firstRun.title')}
            description={t('dashboard.firstRun.description')}
            hint={t('dashboard.firstRun.hint')}
            action={
              <Link to="/onboarding" className={styles.firstRunCta}>
      ...
      <section aria-labelledby="discover-heading" className={styles.discoverSection}>
        <header className={styles.sectionHeader}>
          <h2 id="discover-heading">{t('dashboard.discoverTitle')}</h2>
          <p className={styles.sectionSubtitle}>{t('dashboard.discoverHint')}</p>
        </header>
        <Accordion
          allowMultiple
          headingLevel={3}
          items={[
            {
              id: 'hospitals',
              title: t('dashboard.nearHospitalsTitle'),
              content: (
                <>
                  {hospitalsQuery.isLoading && (
                    <Skeleton variant="rectangular" height={120} lines={2} />
                  )}
                  ...
                  <div className={styles.cardGrid}>
                    {hospitalsQuery.data?.slice(0, 3).map((h) => (
                      <Card key={h.id} padding="md">
                        <Card.Body>
                          <h4 className={styles.itemTitle}>{h.name}</h4>
            ...
            {
              id: 'shops',
              title: t('dashboard.nearShopsTitle'),
              content: (
                <>
                  ...
                  <div className={styles.cardGrid}>
                    {shopsQuery.data?.slice(0, 3).map((shop) => (
                      <Card key={shop.id} padding="md">
                        <Card.Body>
                          <h4 className={styles.itemTitle}>{shop.name}</h4>
                          <p className={styles.itemMeta}>
            ...
            {
              id: 'care',
              title: t('dashboard.careGuideTitle'),
              content: (
                <>
                  ...
                  {careQuery.data && (
                    <Card padding="md">
                      <Card.Body>
                        <h4 className={styles.itemTitle}>{careQuery.data.sections[0]?.title}</h4>
                ...
                <>
                  {diaryEntries.length === 0 ? (
                    ...
                    <div className={styles.cardGrid}>
                      <Card padding="md">
                        <Card.Body>
                          <h4 className={styles.itemTitle}>{t('diary.stats.title')}</h4>
                          <p className={styles.itemMeta}>
                            {t('diary.stats.total')}: {diaryAggregate.total} ·{' '}
                            {t('diary.stats.recent30')}: {diaryAggregate.recent30}
                          </p>
              ...
              content: (
                <>
                  {communitiesQuery.isLoading && (
                    <Skeleton variant="rectangular" height={80} lines={2} />
                  )}
                  ...
                  <div className={styles.cardGrid}>
                    {communitiesQuery.data?.slice(0, 3).map((c) => (
                      <Card key={c.id} padding="md">
                        <Card.Body>
                          <h4 className={styles.itemTitle}>{c.name}</h4>
                          <Badge variant="default">
                            {t(`communities.kind${capitalize(c.kind)}`)}
                          </Badge>
                          <p className={styles.itemDesc}>
                            {c.memberHint ?? c.language.toUpperCase()}
                          </p>
                          <a
            ...
            {
              id: 'adoption',
              title: t('dashboard.adoptionTitle'),
              content: (
                <>
                  ...
                  <div className={styles.cardGrid}>
                    {adoptionQuery.data?.slice(0, 3).map((a) => (
                      <Card key={a.id} padding="md">
                        <Card.Body>
                          <h4 className={styles.itemTitle}>{a.name}</h4>
                          <p className={styles.itemMeta}>
                            {a.region} ·{' '}
                            <Badge variant="default">
                              {t(`adoption.kind${capitalize(a.kind)}`)}
                            </Badge>
                          </p>
            ...
            {
              id: 'funeral',
              title: t('dashboard.funeralTitle'),
              content: (
                <>
                  ...
                  <div className={styles.cardGrid}>
                    {funeralQuery.data?.slice(0, 3).map((f) => (
                      <Card key={f.id} padding="md">
                        <Card.Body>
                          <h4 className={styles.itemTitle}>{f.name}</h4>
            ...
            {
              id: 'external',
              title: t('dashboard.externalTitle'),
              content: (
                <>
                  {externalLinksQuery.isLoading && (
                    <Skeleton variant="rectangular" height={80} lines={2} />
                  )}
                  ...
                  <div className={styles.cardGrid}>
                    {externalLinksQuery.data?.slice(0, 4).map((link) => (
                      <Card key={link.id} padding="md">
                        <Card.Body>
                          <h4 className={styles.itemTitle}>{link.name}</h4>
                          <p className={styles.itemMeta}>
                            <Badge variant="default">
                              {t(`resources.categories.${link.category}`)}
                            </Badge>
                            {link.badge && (
                              <>
                                {' '}
