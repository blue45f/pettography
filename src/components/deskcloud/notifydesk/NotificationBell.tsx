 ...
 * ──────────────────────────────────────────────────────────────────────────
 * npm publish 가 막힌 동안 형제 앱(여기선 pettography)에 그대로 복붙해서 쓰는 버전입니다.
 * 원본: notifydesk/packages/widget/src 의 react.tsx · client.ts · icons.tsx ·
 * styles.ts · relative-time.ts 를 한 파일로 인라인했습니다(워크스페이스 의존
 * @notifydesk/shared 0 — 필요한 DTO 타입을 이 파일에 인라인). 동작/디자인은
 * @notifydesk/widget 의 <NotificationBell> 과 동일합니다.
 *
 * 사용:
 ...
 *
 * 백엔드 계약(publishable·pk_ 키 + Authorization: Bearer):
        ...
        >
          <div className="nd-panel-header">
            ...
            <button
              ...
              onClick={() => {
                setItems((prev) =>
                  prev.map((it) => (it.status === 'read' ? it : markItemRead(it)))
                )
                applyUnread(0)
                client
                  .markAllRead()
