...
| 메인      | `/dashboard`, `/tools`                                                       | 위치 chip, 종 카드, lifecycle rail, **이번 주 활동 strip**, **주의 항목 위젯**(투약·장비·번식·탈피·수질·동면 교차 집계), **오늘의 컨디션 위젯**(health/habitat/budget/supplies), **다가오는 일정 preview**(D-3 toast + browser notification) / **도구 모음 허브**(63개 도구 10개 카테고리 그룹·검색) |
...
| 일상      | `/diary`, `/routine`, `/calendar`                                            | 일지(사진 URL 첨부) / 카테고리별 routine 체크 / health+supplies+registry 통합 timeline + **.ics 캘린더 내보내기**(휴대폰 캘린더 동기화)                                                                                                                                                              |
...
| 💡 광주기·복지·공간 | `/lighting`, `/bcs`, `/enclosure`          | **조명·광주기**(점등 일정·계절별 낮 길이 목표·UVB 가이드·24h 그래프) / **체형 점수**(1~5 BCS·종별 기준·상태·추세) / **사육장 크기**(종별 최소 치수·적정성 판정) |
...
```bash
pnpm verify        # architecture/env validation + format:check + lint + typecheck + test + build
pnpm test:i18n     # ko/en 키 parity (별도 vitest)
pnpm test:node     # scripts/*.test.mjs node:test
...
- `CodeRabbit review gate` — `.coderabbit.yaml`이 활성화되어 있어 PR이 열리면 [CodeRabbit](https://www.coderabbit.ai/) GitHub App이 자동 리뷰를 시작합니다. 이 job은 CodeRabbit이 head SHA에 대해 PR review를 남길 때까지 최대 20분 대기하며, `CHANGES_REQUESTED`이면 실패합니다.
...
5. Required status checks: `Frontend verify`, `Backend verify`, `CodeRabbit review gate` 세 가지 모두 선택.
...
| 변수                  | 기본값                  | 티어     | 설명                                                                |
| --------------------- | ----------------------- | -------- | ------------------------------------------------------------------- |
| `VITE_API_URL`        | (없음)                  | frontend | 설정 시 NestJS API 호출. 미설정 시 mock fallback.                   |
| `VITE_KAKAO_MAP_KEY`  | (없음)                  | frontend | 카카오맵 SDK 키. 미설정 시 placeholder.                             |
| `PORT`                | `3001`                  | backend  | HTTP 포트. PaaS(Render/Fly)가 주입.                                 |
| `CORS_ORIGINS`        | `http://localhost:5173` | backend  | REST + `/consult` 게이트웨이 허용 origin(쉼표 구분). 프로덕션 필수. |
| `API_THROTTLE_TTL_MS` | `60000`                 | backend  | 전역 API rate limit window(ms).                                     |
| `API_THROTTLE_LIMIT`  | `120`                   | backend  | 전역 API rate limit window당 허용 요청 수.                          |

`pnpm run validate:env`가 프론트/백엔드 env 계약을 검증합니다. `VITE_*` 변수는 브라우저 번들에 노출되므로 token/password/secret류 값을 넣으면 실패합니다.

## 배포

전체 배포 절차(프론트=Vercel, 백엔드=Render Docker, 환경변수·GitHub secret·대시보드 단계)는 [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md)를 참고하세요.
