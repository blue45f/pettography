/**
 * 경량 BGM 모듈 — 런타임에 웹 오리진의 /audio/playlist.json을 읽어요.
 * 플레이리스트가 없거나(404/파싱 실패/0곡) 재생 불가면 기능 전체가 숨겨져요.
 * 기본 OFF 토글 · 페이드인/아웃 · 트랙 로테이션. 합성 엔진/외부 라이브러리 없음.
 */
import { WEB_ORIGIN } from './toss'
...
/** 플레이리스트 로드(1회, 캐시). 없으면 available=false로 기능 숨김. */
export function ensurePlaylist(): Promise<void> {
  playlistPromise ??= (async () => {
    try {
      const res = await fetch(`${WEB_ORIGIN}/audio/playlist.json`)
      ...
      const list =
        data && typeof data === 'object' && Array.isArray((data as { tracks?: unknown }).tracks)
          ...
          : []
      tracks = list.map((t) => ({ ...t, src: new URL(t.src, WEB_ORIGIN).href }))
...
/** BGM 토글(사용자 제스처에서만 호출). 기본 OFF, 켜면 페이드인, 끄면 페이드아웃. */
